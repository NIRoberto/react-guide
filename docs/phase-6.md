...
## Assignment — Production-Ready Airbnb App

> Prepare your Airbnb app for production deployment and migrate the listings page to Next.js with Server Components.

### Tasks

1. Add `.env.local` with `VITE_API_URL=http://localhost:3000/api` and `.env.production` with your production API URL
2. Create `src/config/env.ts` that centralizes all `import.meta.env` access with TypeScript types
3. Run `npm run build` — fix any TypeScript errors that appear
4. Add `vercel.json` with the SPA rewrite rule for React Router
5. Write `.github/workflows/deploy.yml` — runs tests, builds, then deploys to Vercel on push to `main`
6. Create a new Next.js project: `npx create-next-app@latest airbnb-nextjs --typescript --tailwind --app`
7. Migrate the listings page to a Server Component — fetch listings directly without `useEffect`
8. Create a `FavoriteButton` Client Component marked with `'use client'`
9. Add `generateStaticParams` to pre-render all listing detail pages at build time
10. Use Next.js `<Image>` for all listing photos with proper `remotePatterns` config
11. Create an API route at `app/api/listings/route.ts` that returns listings from a mock database
12. Add a Server Action for creating a booking

### Starter Code

```ts
// src/config/env.ts (Vite app)
export const config = {
  apiUrl: import.meta.env.VITE_API_URL as string,
...
// airbnb-nextjs/app/listings/page.tsx
export default async function ListingsPage() {
  // TODO: fetch listings from API or DB
  // TODO: render listings grid as Server Component
}

// airbnb-nextjs/app/listings/[id]/page.tsx
export async function generateStaticParams() {
  // TODO: return array of { id: string } for all listings
}

export default async function ListingDetail({ params }: { params: { id: string } }) {
  // TODO: fetch single listing by params.id
  // TODO: render detail view with <Image> component
}

// airbnb-nextjs/components/FavoriteButton.tsx
'use client'
export function FavoriteButton({ listingId }: { listingId: number }) {
  // TODO: useState for saved, render toggle button
}

// airbnb-nextjs/app/api/listings/route.ts
export async function GET(request: Request) {
  // TODO: return listings as JSON
}

// airbnb-nextjs/app/actions/bookings.ts
'use server'
export async function createBooking(formData: FormData) {
  // TODO: create booking, revalidate listings path
}
```

### Expected Output

Production build passes with no TypeScript errors. GitHub Actions runs tests and deploys on push to `main`. Next.js listings page renders on the server — no loading spinner, instant HTML. `FavoriteButton` works as a Client Component. All listing detail pages are pre-rendered at build time. Images load with blur placeholder and are served in WebP format. API route returns listings. Booking form uses a Server Action.
...
| Concept | What it is |
|---------|-----------|
| `npm run build` | Bundles, minifies, and tree-shakes your app for production |
| `VITE_*` env vars | Client-side environment variables — never put secrets here |
| Vercel | Push-to-deploy hosting — auto-detects Vite, handles CDN and SSL |
| GitHub Actions | Automated CI/CD — runs tests and deploys on every push |
| Next.js | React framework with SSR, SSG, file-based routing, and Server Components |
| Server Component | Runs on the server — no JS sent to client, can be async |
| Client Component | Runs in the browser — needs `'use client'`, can use hooks |
| SSR | HTML generated per request — always fresh |
| SSG | HTML generated at build time — fastest, served from CDN |
| ISR | Static pages regenerated in background after a set interval |
| Server Actions | Run server code from a Client Component — no API route needed |
| `<Image>` | Next.js image component — auto-optimizes, lazy loads, prevents layout shift |
