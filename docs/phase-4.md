...
## Assignment — Booking Flow & Test Suite

> Build a full booking flow with TanStack Query for data fetching, a validated multi-step form, and a complete test suite.

### Tasks

1. Install `@tanstack/react-query` and wrap your app in `QueryClientProvider`
2. Create a `useListings` hook using `useQuery` with a 5-minute `staleTime`
3. Create a `useListing(id)` hook for the detail page
4. Build a 4-step booking form: **Dates & Guests → Personal Info → Payment → Confirmation**
5. Validate each step before advancing — show inline error messages
6. Add a file upload input for profile photo with image preview and size validation
7. Implement an optimistic update with `useMutation` — toggle saved state instantly before server responds
8. Handle all states: loading, error, empty, and refetching
9. Write unit tests for `ListingCard` — renders title, price, superhost badge
10. Test the search filter — typing filters by title and location
11. Mock the listings API and test loading + success + error states
12. Write a form validation test — required fields show errors, valid input advances to next step
...
const API = 'https://your-airbnb-api.com'

export function useListings() {
  return useQuery({
    queryKey: ['listings'],
    queryFn: () => fetch(`${API}/listings`).then(r => {
...
function BookingForm({ listingId }: { listingId: number }) {
  const [step, setStep] = useState(0)
  const [data, setData] = useState({ checkIn: '', checkOut: '', guests: 1, name: '', email: '', phone: '', card: '', expiry: '', cvv: '' })
  const [errors, setErrors] = useState<Record<string, string>>({})
  const createBooking = useCreateBooking()
...
Listings load with caching — instant on revisit, background refetch on stale data. Multi-step form validates each step before advancing. File upload shows image preview. Optimistic update toggles saved state instantly. All tests pass with coverage report showing >80%.
